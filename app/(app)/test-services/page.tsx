...
export default function TestServicesPage() {
  ...
  const _testWalletService = async () => {
    if (!testCharacterId) {
      addResult({
        ...
        status: "error",
        message: "Please enter a character ID to test",
      })
      return
    }
    ...
    try {
      ...
      const wallet = await walletService.getWallet(testCharacterId)
  ...
  const _testCharacterService = async () => {
    ...
    try {
      ...
      if (characters.length > 0 && testCharacterId) {
  ...
  const _testInventoryService = async () => {
    ...
    try {
      ...
      const items = await inventoryService.getInventory(testCharacterId)
      ...
      if (items.length > 0) {
        ...
        const totalValue = await inventoryService.calculateTotalValue(testCharacterId)
        addResult({
        ...
        const equipped = await inventoryService.getEquippedItems(testCharacterId)
        addResult({
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-2">
            ...
            <Button onClick={testWalletService} disabled={loading || !testCharacterId} variant="outline">
              Test WalletService
            </Button>
            <Button onClick={testCharacterService} disabled={loading || !user} variant="outline">
              Test CharacterService
            </Button>
            <Button onClick={testInventoryService} disabled={loading || !testCharacterId} variant="outline">
          ...
          {user && !testCharacterId && (
            <Alert>
              <AlertDescription>
                Please enter a character ID to test WalletService and InventoryService
              </AlertDescription>
            </Alert>
          )}
