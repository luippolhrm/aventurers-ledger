...
DECLARE
  ...
  -- Descuento de tienda (characters.shop_bonuses.discount_percent), alineado a TS
  v_shop_bonuses JSONB;
...
BEGIN
  ...
  -- Multiplicador de precio (igual que getShopPriceMultiplier en la app)
  SELECT shop_bonuses INTO v_shop_bonuses
  FROM characters
  WHERE id = p_character_id;

  IF v_shop_bonuses IS NOT NULL AND (v_shop_bonuses->>'discount_percent') IS NOT NULL THEN
    v_discount_pct := (v_shop_bonuses->>'discount_percent')::NUMERIC;
  END IF;

  IF v_discount_pct IS NULL OR v_discount_pct <= 0 OR v_discount_pct >= 100 THEN
    v_price_multiplier := 1;
  ELSE
    v_price_multiplier := GREATEST(0::NUMERIC, 1 - (v_discount_pct / 100));
  END IF;
  ...
  LOOP
    ...
    -- Precio por unidad con descuento (redondeo como en calculateCartTotal)
    v_unit_price := ROUND(v_shop_item.price_in_copper::NUMERIC * v_price_multiplier)::INTEGER;
...
'Procesa una compra completa de forma atómica. Aplica descuento de tienda desde characters.shop_bonuses.discount_percent (coherente con la capa de aplicación). Valida stock y fondos, actualiza inventario, deduce dinero del wallet con algoritmo inteligente, y crea registros históricos. Todo en una transacción.';
